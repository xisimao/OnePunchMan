...
void MqttChannel::StartCore()
{
    ...
    if (_status == MqttStatus::Connected)
    {
        LogPool::Information(LogEvent::Mqtt, "连接到mqtt,ip:",_ip.c_str(),"port:",_port);
    }
    while (!_cancelled)
    {
        if (_status == MqttStatus::Connected)
        {
            ...
            if (i == MOSQ_ERR_NO_CONN)
            {
                ...
                LogPool::Information(LogEvent::Mqtt, "与mqtt断开连接,ip:", _ip.c_str(), "port:", _port);
        ...
        else
        {
            ...
            if (_status == MqttStatus::Connected)
            {
                ...
                LogPool::Information(LogEvent::Mqtt, "与mqtt重连,ip:", _ip.c_str(), "port:", _port);
            }
            else
            {
