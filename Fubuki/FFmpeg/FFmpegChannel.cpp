...
FFmpegChannel::FFmpegChannel(bool debug)
	...
	, _lastframeIndex(0), _frameSpan(0), _bgrHandler(8)
{
...
ChannelStatus FFmpegChannel::InitDecoder(const string& inputUrl)
{
	...
	_bgrBuffer = (uint8_t*)av_malloc(av_image_get_buffer_size(AV_PIX_FMT_NV21, DestinationWidth, DestinationHeight, 1));
	if (av_image_fill_arrays(_bgrFrame->data, _bgrFrame->linesize, _bgrBuffer, AV_PIX_FMT_NV21, DestinationWidth, DestinationHeight, 1) < 0)
	{
		LogPool::Warning(LogEvent::Decode, "av_image_fill_arrays error");
		return ChannelStatus::DecoderError;
	}
	_bgrSwsContext = sws_getContext(_decodeContext->width, _decodeContext->height, _decodeContext->pix_fmt, DestinationWidth, DestinationHeight, AV_PIX_FMT_NV21,
