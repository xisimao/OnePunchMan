...
EventDetector::EventDetector(int width, int height, EncodeChannel* encodeChannel, DataChannel* dataChannel)
	:  _channelIndex(0), _channelUrl(), _width(width), _height(height), _lanesInited(false)
	, _taskId(0), _image(width,height,false),_encodeChannel(encodeChannel), _dataChannel(dataChannel)
{
...
bool EventDetector::LanesInited() const
{
	return _lanesInited;
}
